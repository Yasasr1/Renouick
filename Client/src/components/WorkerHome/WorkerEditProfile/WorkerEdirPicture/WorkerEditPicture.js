...
class WorkerEditPicture extends Component {
    state = {
        url : '',
        publicId: ''
    }
    ...
    picUpdateHandler = (result) => {
        if(result.event === 'success') {
          ...
          axios.post('http://localhost:4000/worker/updatePic', newPic)
          .then(res => {
              alert(res.data);
          })
          .catch(err => {
    ...
    picDeleteHandler = () => {
        ...
        axios.post('http://localhost:4000/worker/deletePic', email)
        .then(res => {
            alert(res.data);
        })
        .catch(err => {
...
export default connect(mapStateToProps,null)(WorkerEditPicture);